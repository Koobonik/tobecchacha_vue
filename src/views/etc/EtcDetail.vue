<template>
  <div>
    ...
    <div style="text-align: start; padding-left: 20px; margin-bottom: 10px;font-size: 16px; letter-spacing: -2.0px; color: grey">{{etc.subTitle}}</div>
      <video style="width: 100%; padding-left: 10px; padding-right: 10px;" v-if="etc.mp4File != null && (etc.mp4File.endsWith('mp4') || etc.mp4File.endsWith('mov') || etc.mp4File.endsWith('wmv') || etc.mp4File.endsWith('avi') || etc.mp4File.endsWith('mpeg') || etc.mp4File.endsWith('f4v'))" autoplay v-bind:src="etc.mp4File" loop controls></video>
    <b-carousel
            v-else
        id="carousel-1"
